...
export class OCRService {
  ...
  private isValidGSTFormat(gst: string): boolean {
    ...
    if (!/^[A-Z]{5}$/.test(gst.substring(2, 7))) return false;
    
    // Characters 8-11 should be digits (PAN last 4 digits)
    if (!/^\d{4}$/.test(gst.substring(7, 11))) return false;
    
    // Character 12 should be a letter (PAN check digit)
    if (!/^[A-Z]$/.test(gst.substring(11, 12))) return false;
    
    // Character 13 should be a digit
    if (!/^\d$/.test(gst.substring(12, 13))) return false;
    
    // Character 14 should be alphanumeric
    if (!/^[A-Z0-9]$/.test(gst.substring(13, 14))) return false;
    
    // Character 15 should be a digit
    if (!/^\d$/.test(gst.substring(14, 15))) return false;
